...
public class HelloController implements Initializable {
    ...
    public Double calculating(String operation, StringBuilder history) {
        try {
            ...
            for (int i = 0; i < operation.length(); i++) {
                ...
                if (i > 0 && c == '-' && checkMinus(operation.charAt(i - 1))) {
                    int z = i;
                    StringBuilder digit = new StringBuilder();
                    while (Character.isDigit(operation.charAt(i + 1)) || operation.charAt(i + 1) == '.') {
                    ...
                    if (!digit.isEmpty()) {
                        double adding = Double.parseDouble((String.valueOf(digit)));
                        adding *= -1;
                        stackDigit.push(adding);
                    }
                }
                if (c == 'P')
                    ...
                    while (!stackOperator.isEmpty() && c != '$' && this.giveValue(stackOperator.peek()) >= this.giveValue(c))
                        this.calculatingStack(stackDigit, stackOperator.pop(), history);
                    if (c != '$')
                        stackOperator.push(c);
                }
    ...
    public void calculatingStack(ArrayStack<Double> stack, char operator, StringBuilder history1) throws Exception {
        ...
        if ((operator != '!' && operator != '-') && stack.isEmpty())
            throw new Exception();
        if (!stack.isEmpty())
        ...
        switch (operator) {
            ...
            case '-':
                if (!stack.isEmpty()) {
                    ...
                    stack.push(-1 * firstDigit);
                    history1.append(-1).append('*').append(firstDigit).append("   ");
                }
                break;
            case '*':
                stack.pop();
    ...
    public boolean checkMinus(char c) {
        if (c == '(' || c == '*' || c == '/' || c == '^')
            return true;
        else return false;
    }
